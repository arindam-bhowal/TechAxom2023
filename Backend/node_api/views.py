...
from node_api.serializers import NoteSerializer, CertificateSerializer
import math
...
class Certificate(generics.GenericAPIView):
    ...
    def get(self, request):
        ...
        total_notes = Certificate.count()
        if search_param:
            Certificate = Certificate.filter(title__icontains=search_param)
        serializer = self.serializer_class(Certificate[start_num:end_num], many=True)
        return Response({
            "status": "success",
            "total": total_notes,
            "page": page_num,
...
class CertificateDetail(generics.GenericAPIView):
    ...
    def get_note(self, pk):
        try:
            return CertificateModel.objects.get(pk=pk)
        except:
            return None
